...
- **NAT Information Elements** — the direct UDP listener (not the goflow2/Vector HTTP path, which normalizes into goflow2's fixed protobuf schema) decodes the standard IANA NAT fields when an exporter sends them (IPFIX `postNATSourceIPv4Address`/`postNATDestinationIPv4Address`/`natEvent`, or NetFlow v9's equivalent `NF_F_XLATE_*` fields) and stores them alongside the flow. Captured only — no page surfaces them.
...
Selecting one of those rows opens pktFlow's **real page** inside pktHub —
...
| `log_forward_app_name` | `pktflow` | APP-NAME in the syslog message |
