...
**Live deployment:** Vyne infrastructure O2 server — `http://172.23.80.5:8080`
...
ssh ec2-user@172.23.80.5   # requires Python+Paramiko (see O2_SSH_CONNECTION.md)
...
- SSH access to O2 requires the `VyneCorpNetInfra.pem` key and SentinelOne EDR blocks the system `ssh.exe` — use Python + Paramiko (see [O2_SSH_CONNECTION.md](O2_SSH_CONNECTION.md))
