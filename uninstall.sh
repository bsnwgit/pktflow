#!/bin/bash
# pktFlow uninstall script — Ubuntu Server 22.04/24.04 LTS
...
# credential encryption key), pktflow.db and its -wal/-shm, logs, backups and
...
# WorkingDirectory, then $PKTFLOW_INSTALL_DIR, then /opt/pktflow.

set -euo pipefail

APP="pktflow"
DISPLAY="pktFlow"
UNIT_PATH="/etc/systemd/system/pktflow.service"
...
# migration — plain *.db does not match "pktflow.db.pre-upgrade".
...
if [ "$PURGE" -eq 1 ]; then
    ...
    for entry in config.yaml pktflow.db logs backups ssl flows.duckdb; do
        if [ -e "$INSTALL_DIR/$entry" ]; then
            echo "    $entry"
        fi
    done
...
# ── ClickHouse ────────────────────────────────────────────────────────────────
# Not dropped automatically, even under --purge. ClickHouse is a separate
# system service that may be shared with the rest of the suite, and dropping a
# database is not something an app uninstaller should decide on its own.
if [ "$PURGE" -eq 1 ] && command -v clickhouse-client &>/dev/null; then
    if clickhouse-client --query "EXISTS DATABASE pktflow" 2>/dev/null | grep -q '^1$'; then
        echo ""
        echo "  ClickHouse database 'pktflow' still exists and was NOT dropped."
        echo "  To remove it as well:"
        echo "      clickhouse-client --query \"DROP DATABASE pktflow\""
    fi
fi
