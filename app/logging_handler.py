"""
SQLite logging handler for pktFlow in-app log viewer.

Intercepts Python log records from the 'pktflow' logger hierarchy and
...
class SQLiteLogHandler(logging.Handler):
    ...
    Usage (called once at app startup):
        handler = SQLiteLogHandler(db_path="/path/to/pktflow.db")
        handler.attach_to_root_logger("pktflow")
    """
    ...
    def attach_to_root_logger(self, logger_name: str = "pktflow") -> None:
    ...
    def set_capture_level(self, level: int) -> None:
        ...
        root = logging.getLogger("pktflow")
        if root.level > level:
            root.setLevel(level)
