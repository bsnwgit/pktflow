...
GET /api/ip-info/asn/{asn} — ipinfo.io's ASN API, for the "ASxxxx" links
rendered wherever an IP lookup surfaces one (e.g. the Org/ASN row above).
"""
...
async def _fetch_ipapi_is(client: httpx.AsyncClient, ip: str, key: str, free_tier: bool, result: IpInfoResult) -> None:
    if not key and not free_tier:
        result.ipapi_is_error = "No ipapi.is key configured — add one in Settings → API Keys, or enable the free tier"
        return
    try:
...
async def _fetch_ipqualityscore(client: httpx.AsyncClient, ip: str, key: str, result: IpInfoResult) -> None:
    if not key:
        result.ipqualityscore_error = "No IPQualityScore key configured — add one in Settings → API Keys"
        return
    try:
...
async def _fetch_mxtoolbox(client: httpx.AsyncClient, ip: str, key: str, result: IpInfoResult) -> None:
    if not key:
        result.mxtoolbox_error = "No MXToolbox key configured — add one in Settings → API Keys"
        return
...
@router.get("/asn/{asn}", response_model=AsnInfoResult)
async def get_asn_info(asn: str, user: CurrentUser):
    asn_norm = asn.strip().upper()
    if not asn_norm.startswith("AS"):
        asn_norm = f"AS{asn_norm}"
    if not asn_norm[2:].isdigit():
        raise HTTPException(status_code=400, detail=f"Invalid ASN: {asn}")

    result = AsnInfoResult(asn=asn_norm)
    ipinfo_key = await _get_user_key(user["username"], "ipinfo")
    if not ipinfo_key:
        result.ipinfo_error = "No ipinfo.io key configured — add one in Settings → API Keys"
        return result
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        try:
            resp = await client.get(f"https://ipinfo.io/{asn_norm}/json", params={"token": ipinfo_key})
            data = resp.json() if resp.headers.get("content-type", "").startswith("application/json") else {}
            if resp.status_code == 200 and "error" not in data:
                result.ipinfo = data
            else:
                # ipinfo.io's ASN endpoint returns errors as a plain string
                # ({"error": "..."}), unlike the IP endpoint's {"error": {"message": "..."}}.
                err = data.get("error")
                result.ipinfo_error = (err.get("message") if isinstance(err, dict) else err) or f"ipinfo.io returned HTTP {resp.status_code}"
        except httpx.RequestError as exc:
            result.ipinfo_error = f"Request error: {exc}"
...
async def get_internal_ip_info(ip: str, user: CurrentUser):
    ...
    client = SuiteClient(integration["base_url"], decrypt_str(integration["suite_token"]),
                         suite_user="pktflow", suite_role="admin")
    try:
