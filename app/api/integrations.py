"""
/api/integrations/* — named connections to sibling pkt* apps pktflow
pulls data from, currently just pktIPAM (its subnet/DHCP/DNS inventory
powers the internal-IP lookup in IpLink, picked by the first enabled
pktipam entry rather than typing base_url/suite_token inline). Separate
from /api/suite/*, which is the INBOUND side (pktHub calling into pktflow).

Setup: on pktIPAM, open Settings -> Integrations -> Suite Integration and
copy its suite token, then paste pktIPAM's base_url and that token here —
you can add as many named pktIPAM instances as you have deployments.
"""
...
async def test_integration(integration_id: int, user: AdminUser, db: aiosqlite.Connection = Depends(get_db)):
    ...
    client = SuiteClient(row["base_url"], decrypt_str(row["suite_token"]), suite_user="pktflow", suite_role="admin")
