...
log = logging.getLogger("pktflow.api.logs")
router = APIRouter()
...
async def get_logs(
    ...
    logger: Optional[str] = Query(None, description="Filter by logger name prefix, e.g. pktflow.ingest"),
...
async def set_log_level(level: str) -> dict:
    ...
    Use 'DEBUG' for deep troubleshooting, reset to 'INFO' when done.
    """
    level = level.upper()
    ...
    try:
        ...
        root_logger = logging.getLogger("pktflow")
