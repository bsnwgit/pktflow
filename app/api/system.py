...
async def system_info(user: CurrentUser) -> dict:
    ...
    return {
        ...
        "github": "https://github.com/bsnwgit/pktflow",
...
def _install_dir() -> Path:
    """Resolve the pktFlow install directory from $PKTFLOW_INSTALL_DIR /
    $PKTFLOW_CONFIG, falling back to the current working directory."""
    env_dir = os.environ.get("PKTFLOW_INSTALL_DIR")
    if env_dir:
        return Path(env_dir)
    env_cfg = os.environ.get("PKTFLOW_CONFIG")
    if env_cfg:
...
def _config_file_path() -> Path:
    ...
    for candidate in [Path("config.yaml"), _install_dir() / "config.yaml"]:
        if candidate.exists():
            return candidate
...
3. Copy pktflow.db    →  <install dir>/pktflow.db
4. Copy config.yaml   →  <install dir>/config.yaml
...
def _restore_from_dir(src_dir: Path, cfg, files: Optional[set[str]]) -> dict:
    ...
    if wanted(flows_name):
        if flows_gz.exists() and flows_gz.stat().st_size > 100:
            try:
                _ok, _err = _clickhouse_import(flows_gz, cfg.clickhouse_database, "flows")
                result[flows_name] = "imported" if _ok else f"error: {_err}"
            except Exception:
                log.exception("ClickHouse restore of %s failed", flows_name)
                result[flows_name] = "error: import failed — see the app log"
        elif flows_plain.exists() and flows_plain.stat().st_size > 0:
            try:
                _ok, _err = _clickhouse_import_plain(
                    flows_plain, cfg.clickhouse_database, "flows"
                )
                result[flows_name] = "imported" if _ok else f"error: {_err}"
            except Exception:
                log.exception("ClickHouse restore of %s failed", flows_name)
...
async def get_port() -> dict:
    """
    The port pktFlow listens on. Lives in config.yaml (startup config, read
...
async def log_forward_reload():
    ...
    configure_forwarding(
        ...
        app_name=str(fwd.get("log_forward_app_name") or "pktflow"),
    )
    return get_forward_stats()
