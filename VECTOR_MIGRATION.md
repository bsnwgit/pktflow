...
type = "http"
inputs = ["add_site"]
uri = "http://10.20.30.5:5080/api/default/medical_netflow/_json"
encoding.codec = "json"
auth.strategy = "basic"
auth.user = "itops@example.com"
...
2. Check ingest stats: `curl http://10.20.30.5:8080/api/ingest/stats`
...
```bash
# On medical (10.20.30.11):
sudo systemctl restart goflow2-vector.service

# On dental (10.20.30.181):
sudo systemctl restart goflow2-vector.service
```
...
curl -s http://10.20.30.5:8080/api/ingest/stats

# Check pktFlow service health
curl -s http://10.20.30.5:8080/api/health
