...
- ✅ **Radar** (`/radar`, in the sidebar nav under Topology) — plan-position scope over the same `/flows/geo` payload the Geo Map uses: origin = spherical centroid of the site-mapped locations, bearing = true initial bearing from it, range = great-circle distance on a log scale (rings at 500/2 000/8 000/20 000 km), blip area = bytes, halo = flows. Arcs keep the colour/dash the backend already resolved from Traffic Rules; markers keep their Site colours; the Line Styles/Sites/NAT Mappings legend and its click-to-filter come from the Geo Map's own `filterGeoData`, not a second implementation. Also the map card on the Analytics dashboard, replacing the Geo Map card there.
