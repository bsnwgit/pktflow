...
pktFlow is a production NetFlow visualization and alerting platform deployed on the Vyne infrastructure O2 server. It receives live NetFlow data from two collectors, stores it in ClickHouse, and serves a React dashboard.

**Live URL:** http://172.23.80.5:8766  
...
|------|----|------|---------|
| O2 Server (pktFlow + ClickHouse) | 172.23.80.5 | ec2-user | `C:\Users\robert.barnett\.ssh\VyneCorpNetInfra.pem` |
...
```
Routers (172.27.28.89/QTS, 192.168.44.7/OneNeck)
    │  NetFlow v9 UDP
    ▼
Medical Collector (172.23.80.11:2055)
...
Vector (same host, stdin source)
    ...
    ▼
pktFlow ingest (172.23.80.5:8766/api/ingest/flows)
...
```json
{
  ...
  "host": "ip-172-23-80-11.internal.vynecorp.com",
  "source_type": "stdin"
}
```
...
- `site` is set to `"oneneck"`, `"qts"`, or `"unknown"` by the vector add_site transform
...
- Command: `/bin/bash -c "/mnt/software/goflow2/goflow2 -format json -listen netflow://172.23.80.11:2055 -addr :8080 | /mnt/software/vector/vector --config /mnt/software/vector/vector.toml"`
...
```
C:\Users\robert.barnett\My Drive\Documents\Claude\Projects\pktFlow\scripts\deploy_frontend.py
```
Run it via Desktop Commander `start_process` (cmd shell):
```
C:\Users\robert.barnett\AppData\Local\Programs\Python\Python313\python.exe "C:\Users\robert.barnett\My Drive\Documents\Claude\Projects\pktFlow\scripts\deploy_frontend.py"
```
...
```
C:\Users\robert.barnett\AppData\Local\Programs\Python\Python313\python.exe "C:\Users\robert.barnett\My Drive\Documents\Claude\Projects\pktFlow\scripts\check_build.py"
```
...
### Backend changes
1. Write/edit local file in `C:\Users\robert.barnett\My Drive\Documents\Claude\Projects\pktFlow\`
...
```
GET http://172.23.80.5:8766/api/ingest/stats
...
- NetFlow ingest from two collectors (medical 172.23.80.11, dental 10.56.57.181) via goflow2 + Vector
