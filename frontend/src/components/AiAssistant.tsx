 ...
 * Accessible from any page. Sends current view's flow context to Claude.
 */
...
interface AiAssistantProps {
  /** Optional context from the current view (devices, top talkers, etc.) */
...
export default function AiAssistant({ context = {} }: AiAssistantProps) {
  ...
  // Check if any AI provider is enabled and configured
  useEffect(() => {
        ...
        <div className="fixed bottom-20 right-6 z-40 w-96 max-w-[calc(100vw-3rem)] bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl flex flex-col"
            ...
            {messages.length === 0 && configured !== false && (
              <div className="text-white text-xs space-y-2">
                ...
                {[
                  ...
                  'Are there any unusual flow patterns right now?',
                  'What does it mean if packets are much higher than bytes?',
                ].map(q => (
                  <button
                    key={q}
                    onClick={() => { setInput(q); }}
          ...
          <div className="px-4 py-3 border-t border-gray-800 flex gap-2">
            <textarea
              ...
              placeholder={configured === false ? 'Configure API key first' : 'Ask about your network traffic…'}
              rows={1}
