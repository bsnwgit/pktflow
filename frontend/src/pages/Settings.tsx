...
// ── Main page ─────────────────────────────────────────────────────────────────
type TabId = 'general' | 'security' | 'data' | 'notifications' | 'apikeys' | 'system' | 'devices' | 'vpnmappings' | 'ingest'
            ...
            {securityTab === 'ai' && (
              <Section title="AI Assistant" onSave={aiAssistantSave.save} saving={aiAssistantSave.saving} saved={aiAssistantSave.saved} error={aiAssistantSave.error}
                help={{
                  ...
                  content: <>
                    <p>Providers are tried top to bottom — <span className="text-gray-300 font-medium">local / self-hosted providers first</span>, since they keep your flow data on hardware you control, then cloud providers as a fallback. Turn on as many as you like; the first enabled provider with valid config answers each question in the chat panel.</p>
                    <p className="mt-2"><span className="text-gray-300 font-medium">Ollama</span> and other local endpoints run entirely on your own hardware — no flow data leaves your network. <span className="text-gray-300 font-medium">Anthropic</span> is a paid, cloud-hosted API and needs its own key from console.anthropic.com, separate from a Claude Enterprise seat.</p>
                  </>,
                }}
              >
                ...
                >
                  ...
                  <MiniField label="Model" hint="Haiku is fast and cost-effective">
                    <SelectInput
