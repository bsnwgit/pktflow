...
function LogForwardTester({ host, port, protocol }: { host: string; port: number; protocol: string }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      setOk(r.ok)
      setResult(r.ok
        ? `Sent 1 message to ${r.target} — check pktLog for "pktFlow log forwarding test message"`
  ...
  return (
    <div>
      <div className="flex items-center gap-2">
        <TextInput value={value} onChange={onChange} placeholder={detected || 'https://pktflow.example.com'} mono />
        <button
          type="button"
...
// ── Main page ─────────────────────────────────────────────────────────────────
type TabId = 'general' | 'security' | 'data' | 'notifications' | 'resonance' | 'apikeys' | 'system' | 'devices' | 'vpnmappings' | 'ingest'
...
const APP_SECTION_LABEL = 'pktFlow'
...
// ── Open-source packages actually used by this app (requirements.txt + package.json) ──
const OSS_NOTICES: Array<{ name: string; license: string }> = [
  ...
  { name: 'Authlib',            license: 'BSD-3-Clause' },
  { name: 'itsdangerous',       license: 'BSD-3-Clause' },
  { name: 'httpx',              license: 'BSD-3-Clause' },
  { name: 'python3-saml',       license: 'MIT' },
  { name: 'APScheduler',        license: 'MIT' },
  ...
  { name: 'python-netflow-v9',  license: 'MIT' },
      ...
      {dataTab === 'logforward' && (
        <Section title="Log Forwarding" onSave={logForwardSave.save} saving={logForwardSave.saving} saved={logForwardSave.saved} error={logForwardSave.error}
          help={{
            ...
            content: <>
              <p>Ships pktFlow's own application log to a syslog collector — normally <span className="text-gray-300 font-medium">pktLog</span>, which listens on port <code className="text-gray-400">5514</code> — so this app's logs sit alongside the rest of the estate instead of only in its local Logs page.</p>
              ...
              <p>Delivery is fire-and-forget on a background thread — if the collector is unreachable, lines are dropped and counted rather than blocking or crashing pktFlow. Use <span className="text-gray-300 font-medium">Send test message</span> to confirm the path end to end.</p>
        ...
        >
          ...
          <Field label="Application name" hint="Appears as the APP-NAME field in the syslog message">
            <TextInput value={str('log_forward_app_name') || 'pktflow'} onChange={v => set('log_forward_app_name', v)} placeholder="pktflow" />
          </Field>
          <LogForwardTester
      ...
      {tab === 'resonance' && (
        ...
        <Section title="Resonance" onSave={resonanceSave.save} saving={resonanceSave.saving} saved={resonanceSave.saved} error={resonanceSave.error}
          help={{
            ...
            content: <>
              <p>Resonance is the shared assistant surface for the pkt suite. It mounts as a launcher in the corner of every page, but the assistant itself runs on the resonance server rather than inside pktFlow.</p>
              ...
              <p><span className="text-gray-300 font-medium">pktFlow&rsquo;s own address</span> is what a browser types to reach this app, and it is the string that has to appear on the resonance key&rsquo;s origins list. Leave it blank and pktFlow works it out from the request — correct for a direct install, and wrong behind a reverse proxy, where it sees the internal address rather than the one users type.</p>
              <p><span className="text-gray-300 font-medium">pktFlow never sends your login credentials.</span> It vouches for whoever is signed in and receives a short-lived, single-use code the browser spends on opening the widget. The key below never reaches the browser.</p>
              <p><span className="text-gray-300 font-medium">The panel&rsquo;s insides belong to resonance.</span> It is an iframe served from resonance&rsquo;s own origin, so pktFlow cannot restyle it or move its controls — where the buttons sit is a resonance change. What pktFlow can do is make the panel bigger, with <span className="text-gray-300 font-medium">Panel size</span>, which is usually what &ldquo;more room to read&rdquo; actually needs.</p>
              ...
              <p><span className="text-gray-300 font-medium">The assistant can read this install&rsquo;s data.</span> Individual flow records, the top talkers in a window, the exporters sending flow data, the configured sites and their ranges, the collection summary, alert rules and the alerts they have fired, and pktFlow&rsquo;s own diagnostic log. Each call is made by this page on the session of whoever is signed in, so it reaches only what that person could already open. The list is published at <code>/.well-known/resonance.json</code> and is fixed in the code rather than configurable — but it is inert unless <span className="text-gray-300 font-medium">Enabled</span> is on and the person&rsquo;s role is above <span className="text-gray-300 font-medium">No access</span> below.</p>
              <p><span className="text-gray-300 font-medium">Flow search is capped harder than anything else in the suite.</span> A window is always applied, the page is small, and the true match count comes back beside it, so the assistant says &ldquo;eleven thousand matched, here are twenty-five&rdquo; rather than passing off a page as the answer.</p>
              <p><span className="text-gray-300 font-medium">Read and write adds three operations, and no more.</span> Acknowledge one alert, acknowledge all of them, and switch an existing alert rule on or off. Nothing it can call purges a sampler&rsquo;s history, changes retention, or edits a site, NAT mapping or traffic rule. Resonance stops and reads the real values back to the person before running any of them.</p>
              <p><span className="text-gray-300 font-medium">A level never exceeds the role.</span> Two checks have to agree: the level set here, and pktFlow&rsquo;s own rule for the thing being done — switching a rule is an analyst&rsquo;s to do, so a viewer on <span className="text-gray-300 font-medium">Read and write</span> still cannot.</p>
        ...
        >
          ...
          <Field label="pktFlow's own address" hint="What browsers type to reach pktFlow. Copy it onto the resonance key.">
          ...
          <Field label="What each role can do" hint="No access hides the launcher entirely. Read only lets the assistant look. Read and write also lets it act — never beyond what that role can already do in pktFlow.">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            ...
            <div className="px-6 py-4">
              <p className="text-xs text-gray-400 mb-3">
                {systemInfo?.app_name ?? 'pktFlow'} is built with the following open-source software:
              </p>
...
function SitesSection({ isAdmin }: { isAdmin: boolean }) {
  ...
  const blank: SiteIn = { name: '', display_name: '', fill_color: '#8ad8ea', stroke_color: '#63c3d8', badge_bg: '#2a2418', badge_text: '#dcd6c9', show_in_legend: true, ip_cidr: '' }
